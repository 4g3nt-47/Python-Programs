...
def drawGLScene():
        ...
        if ret == True:
                ...
                glMatrixMode(GL_MODELVIEW)
                glLoadIdentity()
                ar_list = detect_markers(frame, camera_matrix, dist_coeff)
                for i in ar_list:
                        if i[0] == 8:
...
def detect_markers(img, camera_matrix, dist_coeff):
        ...
        for q in corners:
            ...
            x = int((m1*q[0][3][0] - m2*q[0][2][0] + q[0][2][1] - q[0][3][1])/(m1 - m2));
            y = int(m1*(x - q[0][3][0]) + q[0][3][1]);
            centre.append((x,y))
        for d in idd:
...
def draw_background(img):
        ...
        bg_image = Image.fromarray(bg_image,"RGB")
        cv2.imwrite("first.png", img)
        textureSurface = pygame.image.load('first.png')
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, 0, width, height, 0, 0,GL_UNSIGNED_BYTE, textureData)
        glEnable(GL_TEXTURE_2D)'''
        glBegin(GL_QUADS)
        # convert image to OpenGL texture format
        bg_image = cv2.flip(img, 0)
        img = pygame.image.load('first.png')
        bg_image = Image.fromarray(bg_image)     
        ix = bg_image.size[0]
        iy = bg_image.size[1]
        bg_image = bg_image.tostring(img, "RGB", 1)
